...
支持四套皮肤：深海薄荷、奶油拿铁、午夜冰蓝、暮色鸢尾。右键小组件 → 小组件偏好 → 小组件皮肤，点击立即应用并自动保存。小组件、右键菜单与偏好窗口跟随配色；收入参数后台保持原有样式。

小改进：收入计算由前后端共用；仅返回收入/进度的外部 API 会直接显示其数值。班次支持以 `24:00` 结束，拒绝 `24:01` 等无效时间。状态会在上班前、午休和下班后自动变化，工作期间保留自定义文案。保存配置时保留 `config.json.bak`，主配置损坏时自动读取备份。运行 `npm test` 可验证计算、迁移和恢复场景。
...
![小组件偏好（点击皮肤立即应用并自动保存）](assets/screenshot-theme-picker.png)
